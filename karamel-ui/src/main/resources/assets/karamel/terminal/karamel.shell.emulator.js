      ...
      function($scope, $log, $interval, terminalConfiguration, promptCreator, sshIps, KaramelCoreRestServices) {
        ...
        $scope.$on('core-result-ssh', function(e, response) {
          if ($scope.machine && $scope.machine != response.context)
            $scope.switchMachine = true;
          else
            $scope.switchMachine = false;
          $scope.machine = response.context;
          if (!$scope.resultMap[$scope.machine])
            $scope.resultMap[$scope.machine] = [];
          ...
          if (result !== null && result !== '') {
            ...
            if (!prompt || 0 === prompt.length)
              prompt = response.context + ">";
            $scope.prompt.text = prompt;
            var items = bashColorsToHtml(text);
            $scope.resultMap[$scope.machine].push({
              added: true,
              output: true,
              items: items
            });
        ...
        function bashColorsToHtml(str) {
          var pattern = new RegExp(/(\033\[((?:[0,3,4]?[0-7];)*(?:[0,3,4]?[0-7]))m([^\[]*?)\033\[0m)/g);

          var match;
          var lastInx = -1;
          var newStr = [];
          while (match = pattern.exec(str)) {
            var styles = match[2].split(";");
            var style = "";
            for (var i in styles) {
            ...
            if (lastInx < match.index) {
              var other = str.substring(lastInx, match.index);
              var cleaned = other.replace("\033\[0m", "");
              if (cleaned.length > 0)
                newStr.push({style: "", text: cleaned});
            }
            lastInx = pattern.lastIndex;
            newStr.push({style: style, text: match[3]});
          }
          if (str !== null && lastInx < str.length) {
            newStr.push({style: "", text: str.substring(lastInx, str.length)});
          }
          return newStr;
        }
          ...
          return {
            ...
            post: function postLink(scope, element, attrs, controller) {
              ...
              scope.$watchCollection(function() {
                return scope.resultMap[scope.machine];
              }, function(newValues, oldValues) {

                if (scope.switchMachine || !oldValues || (oldValues.length && !newValues.length)) { // removal detected
                ...
                if (newValues) {
                  for (var j = 0; j < newValues.length; j++) {
                    ...
                    if (!scope.switchMachine && newValue.displayed)
                      continue;
