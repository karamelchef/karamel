!function() {
  "use strict";
  angular.module("ui.sortable", []).constant("sortableConfig", {itemClass: "as-sortable-item", handleClass: "as-sortable-item-handle", placeHolderClass: "as-sortable-placeholder", dragClass: "as-sortable-drag", hiddenClass: "as-sortable-hidden"})
}(), function() {
  "use strict";
  var mainModule = angular.module("ui.sortable");
  mainModule.factory("$helper", ["$document", "$window", function($document, $window) {
      return{height: function(element) {
          ...
          var pos = {};
          return pos.offsetX = event.pageX - this.offset(target).left, pos.offsetY = event.pageY - this.offset(target).top, pos.startX = pos.lastX = event.pageX, pos.startY = pos.lastY = event.pageY, pos.nowX = pos.nowY = pos.distX = pos.distY = pos.dirAx = 0, pos.dirX = pos.dirY = pos.lastDirX = pos.lastDirY = pos.distAxX = pos.distAxY = 0, pos
          ...
          return{index: item.index(), parent: item.sortableScope, source: item, sourceInfo: {index: item.index(), itemScope: item.itemScope, sortableScope: item.sortableScope}, moveTo: function(parent, index) {
  ...
  mainModule.controller("ui.sortable.sortableController", ["$scope", function($scope) {
    ...
    return{require: "ngModel", restrict: "A", scope: !0, controller: "ui.sortable.sortableController", link: function(scope, element, attrs, ngModelController) {
        ...
        }, scope.element = element, callbacks = {accept: null, orderChanged: null, itemMoved: null, dragStart: null, dragCancel: null, dragEnd: null}, callbacks.accept = function() {
          return!0
  ...
  mainModule.controller("ui.sortable.sortableItemHandleController", ["$scope", function($scope) {
      ...
      return{require: "^asSortableItem", scope: !0, restrict: "A", controller: "ui.sortable.sortableItemHandleController", link: function(scope, element, attrs, itemController) {
          ...
          var dragElement, placeHolder, placeElement, itemPosition, dragItemInfo, containment, dragStart, dragMove, dragEnd, dragCancel, isDraggable, isDragBefore, isPlaceHolderPresent, bindDrag, unbindDrag, bindEvents, unBindEvents, hasTouch, dragHandled, isEnabled = !0;
          hasTouch = $window.hasOwnProperty("ontouchstart"), sortableConfig.handleClass && element.addClass(sortableConfig.handleClass), scope.itemScope = itemController.scope, scope.$watch("sortableScope.isEnabled", function(newVal) {
            ...
            if ((!hasTouch || !$helper.isTouchInvalid(event)) && dragHandled && dragElement) {
              if (event.preventDefault(), eventObj = $helper.eventObj(event), $helper.movePosition(eventObj, dragElement, itemPosition, containment), targetX = eventObj.pageX - $document[0].documentElement.scrollLeft, targetY = eventObj.pageY - ($window.pageYOffset || $document[0].documentElement.scrollTop), dragElement.addClass(sortableConfig.hiddenClass), $document[0].elementFromPoint(targetX, targetY), targetElement = angular.element($document[0].elementFromPoint(targetX, targetY)), dragElement.removeClass(sortableConfig.hiddenClass), targetScope = targetElement.scope(), !targetScope || !targetScope.type)
                return;
              if ("handle" === targetScope.type && (targetScope = targetScope.itemScope), "item" !== targetScope.type && "sortable" !== targetScope.type)
                return;
            ...
            element.bind("touchstart", dragStart), element.bind("mousedown", dragStart)
          }, unbindDrag = function() {
            element.unbind("touchstart", dragStart), element.unbind("mousedown", dragStart)
  ...
  mainModule.controller("ui.sortable.sortableItemController", ["$scope", function($scope) {
      ...
      return{require: "^asSortable", restrict: "A", controller: "ui.sortable.sortableItemController", link: function(scope, element, attrs, sortableController) {
