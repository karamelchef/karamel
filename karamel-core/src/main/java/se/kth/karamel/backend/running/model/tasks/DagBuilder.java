...
public class DagBuilder {
  ...
  public static Dag getInstallationDag(JsonCluster cluster, ClusterRuntime clusterEntity, TaskSubmitter submitter, Map<String, JsonObject> chefJsons) throws KaramelException {
    Dag dag = new Dag();
    ...
    Map<String, Map<String, Task>> rlts = recipeLevelTasks(cluster, clusterEntity, chefJsons, submitter, allRecipeTasks, dag);
    updateKaramelDependecnies(allRecipeTasks, dag, rlts);
    return dag;
  }

  private static boolean updateKaramelDependecnies(Map<String, RunRecipeTask> allRecipeTasks, Dag dag, Map<String, Map<String, Task>> rlts) throws KaramelException {
  ...
  public static Map<String, Map<String, Task>> recipeLevelTasks(JsonCluster cluster, ClusterRuntime clusterEntity,
          Map<String, JsonObject> chefJsons, TaskSubmitter submitter, Map<String, RunRecipeTask> allRecipeTasks, Dag dag) throws KaramelException {
      ...
      for (MachineRuntime me : ge.getMachines()) {
        for (JsonCookbook jc : jg.getCookbooks()) {
          ...
          for (JsonRecipe rec : jc.getRecipes()) {
            ...
            makeRecipeTask(rec.getCanonicalName(), me, map, json1, submitter, urls.id, jc.getName(), allRecipeTasks, dag);
          }
        }
      }
    }
  ...
  private static RunRecipeTask makeRecipeTask(String recipeName, MachineRuntime machine,
          ...
          String cookbookId, String cookbookName, Map<String, RunRecipeTask> allRecipeTasks, Dag dag) throws DagConstructionException {
    RunRecipeTask t1 = makeRecipeTask(recipeName, machine, chefJson, submitter, cookbookId, cookbookName, allRecipeTasks, dag);
  ...
  private static RunRecipeTask makeRecipeTask(String recipeName, MachineRuntime machine, JsonObject chefJson,
          TaskSubmitter submitter, String cookbookId, String cookbookName, Map<String, RunRecipeTask> allRecipeTasks, Dag dag) throws DagConstructionException {
  ...
  public static Map<String, Map<String, Task>> cookbookLevelTasks(JsonCluster cluster, ClusterRuntime clusterEntity,
    ...
    for (GroupRuntime ge : clusterEntity.getGroups()) {
      ...
      for (MachineRuntime me : ge.getMachines()) {
        ...
        for (JsonCookbook jc : jg.getCookbooks()) {
          CookbookUrls urls = jc.getUrls();
          VendorCookbookTask t1 = new VendorCookbookTask(me, submitter, urls.id, Settings.COOKBOOKS_ROOT_VENDOR_PATH, urls.repoName, urls.home, urls.branch);
          dag.addTask(t1);
          map1.put(t1.uniqueId(), t1);
          ...
          RunRecipeTask t2 = makeRecipeTask(recipeName, me, json, submitter, urls.id, jc.getName(), allRecipeTasks, dag);
          map1.put(t2.uniqueId(), t2);
        }
        logger.debug(String.format("Cookbook-level tasks for the machine '%s' in the group '%s' are: %s", me.getPublicIp(), ge.getName(), map1.keySet()));
  ...
  public static void machineLevelTasks(JsonCluster cluster, ClusterRuntime clusterEntity, TaskSubmitter submitter, Dag dag) throws KaramelException {
