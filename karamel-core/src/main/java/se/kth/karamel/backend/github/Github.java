...
public class Github {
  ...
  private static void addCommitPushRepo(String repoName, String sshUrl) throws IOException {
    File targetDir = new File(Settings.COOKBOOK_DESIGNER_PATH );
    if (targetDir.exists() == false) {
      targetDir.mkdirs();
    }
    File f = new File(Settings.COOKBOOK_DESIGNER_PATH + File.separator + repoName );

    Git git = null;
    try {
//      .setBranch("master")
      git = Git.cloneRepository().setURI(sshUrl)
          .setDirectory(f)
          .setCredentialsProvider(new UsernamePasswordCredentialsProvider(user, password)).call();
//      git.checkout().addPath(Settings.COOKBOOK_DESIGNER_PATH + File.separator + repoName ).setName("master").
//          call();

      CookbookScaffolder.create(repoName);
      git.add().addFilepattern("metadata.rb").addFilepattern("Berksfile").addFilepattern("Karamelfile")
  ...
  public synchronized static void createRepoForOrg(String org, String repoName, String description) throws IOException {
    ...
    addCommitPushRepo(repoName, r.getSshUrl());
  }
